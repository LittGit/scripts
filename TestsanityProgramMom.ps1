...
function Get-MergedEvents {
    ...
    # Merge events
    $mergedEvents = Merge-Events -SanityEvents $sanityEvents -MomentusEvents $script:momentusEventsCache
...
while ($running) {
    ...
    switch ($choice) {
        "1" {
            ...
            $events = Get-MergedEvents -StartDate $startDate -EndDate $endDate
        ...
        "6" {
            ...
            if ($searchTerm) {
                ...
                $events = Get-MergedEvents -StartDate $startDate -EndDate $endDate -SearchTerm $searchTerm
        ...
        "8" {
            ...
            # Tøm event details cache for å tvinge refresh
            $script:eventDetailsCache = @{}
